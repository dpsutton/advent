...
(defn line->node [line]
  (let [[node weight & edges] (remove str/blank?
                                    (-> line
                                        (str/replace #"\(|\)" "")
                                        (str/replace #"->" "")
                                        (str/replace #"," "")
                                        (str/split #"\s")))]
    [(keyword node) {:weight (Integer/parseInt weight)
                     :edges (mapv keyword edges)}]))

(defn vector-line
  "S contains a ->"
  [s]
  (let [[node arrow-and-edges] (str/split s #"->")]
    [(keyword (str/trim node)) (map (comp keyword str/trim) (rest (str/split arrow-and-edges #"\s")))]))
...
(def sample (as-graph sample-input))
(def data (as-graph data-input))
