...
(defn transform [line]
  (let [parts (str/split (str/trim line) #"\s")]
    (map read-string parts)))

#_(defn transform [line]
    (map symbol (str/split (str/trim line) #"\s")))

(defn transform-input [input]
  (map transform (str/split-lines input)))

(def sample
  (transform-input sample-text))
...
(def data (transform-input input-text))
