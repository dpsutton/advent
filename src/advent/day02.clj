...
(defn read-spreadsheet [spreadsheet]
  (->> spreadsheet
       str/split-lines
       (map str/trim)
       (map #(str/split % #"\s+"))
...
(defn checksum [row-fn spreadsheet]
  (let [spreadsheet (read-spreadsheet spreadsheet)]
    (apply + (map row-fn spreadsheet))))

(defn min-max [row]
  (- (apply max row) (apply min row)))

(defn evenly-divides [row]
  (let [choices    (mapcat (fn [d] (map #(vector d %) row)) row)
        [high low] (first (filter (fn [[high low]]
                                    (and (not= high low) (= 0 (rem high low))))
                                  choices))]
       (/ high low)))

(deftest checksum-tests
  (let [spreadsheet "5 1 9 5 \n 7 5 3 \n 2 4 6 8"]
    (is (= 18 (checksum min-max spreadsheet))))
  (let [spreadsheet "5 9 2 8\n9 4 7 3\n3 8 6 5"]
    (is (= 9 (checksum evenly-divides spreadsheet)))))

(deftest evenly-divides-tests
  (are [row output] (= (evenly-divides row) output)
    [5 9 2 8] 4
    [9 4 7 3] 3
    [3 8 6 5] 2))
